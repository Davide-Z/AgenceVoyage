...
//---------- Formulaire Etape
     ...
     * Affichage d'un formulaire d'ajout de nouvelle étape (GET)
     */
    $admin_programmationnew_getaction = function($circuit_id) use ($app)
    {
        $circuit = get_circuit_by_id( $circuit_id );
    ...
    // GET
    $app->get('/admin/programmationnew/{circuit_id}', $admin_programmationnew_getaction)
     ...
     * Soumission d'un formulaire d'ajout de nouvelle étape (POST)
     */
    $admin_programmationnew_postaction = function(Request $request, $circuit_id) use ($app)
    {
        $circuit = get_circuit_by_id( $circuit_id );
        ...
        if ($form->isValid())
        {
            ...
            // Ajout de l'étape (calcul des attributs, etc)
            $programmation = $circuit->addProgrammation(new ProgrammationCircuit($data['dateDepart'], $data['nombrePersonnes'], $data['prix'], $data['circuit']));
            
            // Persistence en base de données
            add_programmation($circuit, $programmation->getDateDepart(),
                $programmation->getNombrePersonnes(),
                $programmation->getPrix());
            // the duration has changed, so better save it
            save_circuit($circuit);
    ...
    // POST
    $app->post('/admin/programmationnew/{circuit_id}', $admin_programmationnew_postaction);
    ...
    $admin_programmationdelete_action = function ($id) use ($app) {
        ...
        $circuit = $programmation->getCircuit();
        $found = $circuit->removeProgrammation($programmation);
    ...
    $app->delete('/admin/programmation/{id}', $admin_programmationdelete_action)
    ...
    $admin_programmationmodify_action = function (Request $request, $id, $progid) use ($app) {
        
        $circuit = get_circuit_by_id($id);
        $programmation = get_programmation_by_id($progid);
    ...
    // handle both GET and POST
    $app->match('/admin/programmationmodify/{id}/{progid}', $admin_programmationmodify_action)
